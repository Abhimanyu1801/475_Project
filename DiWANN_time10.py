...
def levenshteinDistance(s1,s2):
    ...
    if len(s1) > len(s2):
        s1,s2 = s2,s1
    distances = range(len(s1) + 1) #suppose the len of s1 is 10, then distance is array with values from 0 to 9 
    for index2,char2 in enumerate(s2): #[int index, char2]  example[1,m,2,k]
        newDistances = [index2+1] #[545]
...
def makeGraphEfficiently(species, repeatmatrix, filename, sparse=False):
	...
	#summary (G)
# 	labelss=['SARS-CoV-2', 'SARS-CoV-2','SARS-CoV-2', 'SARS-CoV-2','SARS-CoV-2', 'SARS-CoV', 'SARS-CoV', 'SARS-CoV', 'SARS-CoV', 'SARS-CoV']
# 	names=[]
# for r in labelss:
#     names.append('; '.join(r))
#     G.vs["label"] = names
...
repmatrix = createMinMatrix(arr)
print(repmatrix)
graph_temp = makeGraphEfficiently(arr,repmatrix,'475proj')
summary(graph_temp)
import networkx as nx
import matplotlib.pylab as plt
g = nx.read_gml('475projDWRNgraphEff.gml', label='id')
nx.draw(g,with_labels =True,font_color="yellow", font_weight="bold")
